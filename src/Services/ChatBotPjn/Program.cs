...
app.MapGet("/", () => Results.Ok(new { ok = true, service = "MiniApi", ts = DateTime.UtcNow }));
...
app.MapGet("/todos/{id:int}", (int id) =>
    todos.FirstOrDefault(t => t.Id == id) is { } found ? Results.Ok(found) : Results.NotFound());
...
{
    var model = req.Model ?? Environment.GetEnvironmentVariable("OPENAI_MODEL") ?? "gpt-4o-mini";
    ...
    if (!resp.IsSuccessStatusCode)
        return Results.Problem(title: "OpenAI error", detail: json, statusCode: (int)resp.StatusCode);
